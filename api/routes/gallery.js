...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // CloudinaryStorage already gives us the public CDN URL.
      ...
      // CloudinaryStorage handles the uploaded asset; do not treat the URL as a local file path.
  ...
  async (req, res) => {
    try {
      ...
      const imageDocuments = req.files.map((file) => ({
        title:       file.originalname.replace(/\.[^/.]+$/, '').replace(/-/g, ' '),
        description: '',
        category:    category?.toLowerCase().trim() || 'other',
      ...
      // CloudinaryStorage manages uploaded assets; there are no local files to unlink here.
